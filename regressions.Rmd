...
lossGivenDefault <- cv.glmnet(x = data.matrix(chargeoff_train_indep), y = chargeoff_train$GrossChargeOffAmount)
...
train_pruned_surv$status <- is.na(train_pruned_surv$EndDate)

train_pruned_surv$EndDate[which(train_pruned_surv$status == 1)] <- as.Date('3/15/16', format="%m/%d/%y")

train_pruned_surv$YearsBeforeDefault <-(as.Date(as.character(train_pruned_surv$EndDate), format = "%m/%d/%y")-as.Date(as.character(train_pruned_surv$ApprovalDate), format="%m/%d/%y")) / 365
...
coxfit1 <- coxph(Surv(as.numeric(train_pruned_surv$YearsBeforeDefault), train_pruned_surv$status)~GrossApproval, data=train_pruned_surv)
summary(coxfit1)
coxfit2 <- coxph(Surv(as.numeric(train_pruned_surv$YearsBeforeDefault), train_pruned_surv$status)~GrossApproval+FEDFUNDS, data=train_pruned_surv)
summary(coxfit2)
coxfit3 <- coxph(Surv(as.numeric(train_pruned_surv$YearsBeforeDefault), train_pruned_surv$status)~GrossApproval+FEDFUNDS+UnemploymentRate, data=train_pruned_surv)
summary(coxfit3)
coxfit4 <- coxph(Surv(as.numeric(train_pruned_surv$YearsBeforeDefault), train_pruned_surv$status)~GrossApproval+FEDFUNDS+UnemploymentRate+PublicDebt.GDP, data=train_pruned_surv)
summary(coxfit4)
