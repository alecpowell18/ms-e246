...
#There's a bug that for some reason fixed itself after I ran this code for the 5th time
...
get.default.probs <- function(pool, cox_coef, t){
  hazard <- exp(data.matrix(pool) %*% cox_coef)
...
hadZeroChargeOff <- cv.glmnet(x = data.matrix(chargeoff_train_indep), y = chargeoff_train_zeros, family = "binomial")
...
plot(train_zero_roc, col = "blue", main="Predicting Zero Chargeoff: \nIn-Sample (blue) and Out of Sample (red)")
...
#random_sample_survvars <-selected_surv_indep[sample_indices, ]
# On 1 year horizon
#random_sample.probs <- get.default.probs(random_sample_survvars, betas, 1)
# On 5 year horizon
#random_sample.probs <- get.default.probs(random_sample_survvars, betas, 5)
...
for(i in 1:20000) {
  ...
  defaultVector[i] <- sum(random_sample.bin == TRUE)
...
library(MASS)
#lda.fit = lda(defaulted~GrossApproval+log(UnemploymentRate)+HousingPriceIndex+FEDFUNDS, data = train_pruned)
...
##we eventually decided to manually update the parameters
